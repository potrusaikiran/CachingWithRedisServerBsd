...
using WebApi.RedisCache.Core;
using WebApi.RedisCache.Core.Cache;
using WebApi.RedisCache.Core.Time;
...
namespace WebApi.RedisCache.V2
{
    ...
    public class CacheOutputAttribute : ActionFilterAttribute
    {
        ...
        protected virtual bool IsCachingAllowed(HttpActionContext actionContext, bool anonymousOnly)
        {
            ...
            //{
            //    if (section.Ziraff[i].IsRedisEnabled != true)
            //    {
            //        return false;
            //        break;
            //    }
        ...
        public override void OnActionExecuting(HttpActionContext actionContext)
        {
            try
            {
                ...
                var contenttype = _webApiCache.Get<MediaTypeHeaderValue>(cachekey + Constants.ContentTypeKey) ??
                                  new MediaTypeHeaderValue(cachekey.Split(new[] { ':' }, 2)[1].Split(';')[0]);
